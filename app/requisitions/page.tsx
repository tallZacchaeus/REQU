  ...
  return (
    <>
      <header className="border-hairline bg-card sticky top-0 z-20 border-b">
        <div className="flex h-14 items-center justify-between px-4">
          ...
          <Link
            ...
            className="bg-primary text-primary-foreground hover:bg-primary/90 -mr-1 flex h-9 cursor-pointer items-center gap-1.5 rounded-lg pr-3 pl-2.5 text-[13.5px] font-semibold transition-colors duration-200"
          >
        ...
        <div className="-mx-px overflow-x-auto px-4 pb-2.5 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            ...
            {FILTERS.map((f) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "flex h-8 cursor-pointer items-center gap-1.5 rounded-lg border px-2.5 text-[13px] font-medium transition-colors duration-200",
                    selected
                      ? "border-primary bg-primary text-primary-foreground"
                      : "border-hairline bg-card text-ink-soft hover:border-ink-faint/40",
                  )}
                >
                  {f.label}
                  <span className={cn("text-[11.5px]", selected ? "text-primary-foreground/70" : "text-ink-faint")}>
                    {count}
                  </span>
                </button>
              )
      ...
      <div className="px-4 pt-4 pb-8">
        ...
        {visible.length === 0 ? (
          <div className="border-hairline mt-6 flex flex-col items-center rounded-xl border border-dashed px-6 py-12 text-center">
